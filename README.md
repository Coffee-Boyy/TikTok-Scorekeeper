...
The Connection tab accepts an optional Euler Stream API key for the standalone gift listener. [Euler Stream's quickstart](https://www.eulerstream.com/docs/api/quickstart) explains how to create an account and generate a key in its dashboard; [plan details](https://www.eulerstream.com/pricing) show route access, which may require a paid plan. The key is encrypted with the operating system's secure storage and kept outside `settings.json`. A saved key appears as `****` in the input placeholder; leave the field blank to retain it, or select **Remove saved API key** to return to community signing limits. A changed key takes effect on the next connection attempt; an in-progress retry starts again immediately. The app also saves a signing-service rate-limit cooldown across restarts and waits until the service's retry time before requesting another connection. During development, file changes no longer automatically reload or restart the app, avoiding blank windows and unnecessary signing requests. Restart manually to load code changes.
...
The configured Windows build target is a portable executable. Ordinary `main` and pull-request builds are unsigned test artifacts. Version-tagged releases must be signed with a trusted Windows code-signing certificate: add the base64-encoded `.pfx` as the repository secret `WIN_CSC_LINK` and its password as `WIN_CSC_KEY_PASSWORD`. On Windows, encode a PFX with `[Convert]::ToBase64String([IO.File]::ReadAllBytes('C:\path\to\certificate.pfx'))` and paste the result into the secret; never commit the certificate or password. The workflow passes these secrets only to the tag build, requires Electron Builder to sign, and verifies both the packaged app and portable executable before publishing. If either secret is missing or signing fails, the release is not published.

The [Windows build workflow](.github/workflows/build-windows.yml) runs on pushes to `main`, pull requests, version tags, and manual dispatch. It installs dependencies from `package-lock.json`, runs the tests, packages the x64 portable `.exe`, verifies the output, and uploads it as the `TikTok-Scorekeeper-Windows-x64` artifact. Pushing a `v*` tag publishes the signed executable on the [GitHub Releases page](https://github.com/Coffee-Boyy/TikTok-Scorekeeper/releases) only after signature verification. For unsigned test builds, open the repository's **Actions** tab, select a successful **Build Windows executable** run, and download the artifact at the bottom of the run page. Extract the artifact ZIP and run the `.exe`; installation is not required. The existing `v0.1.0` release predates this signing requirement and remains unsigned.

A trusted signature establishes the publisher and helps Windows trust future builds, but [Microsoft says a newly signed app may still trigger SmartScreen](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/smartscreen-reputation) until it gains reputation. A self-signed certificate does not solve this. If Defender Antivirus names a malware or potentially unwanted app detection rather than showing an unknown-publisher warning, [submit the file and detection to Microsoft for analysis](https://learn.microsoft.com/en-us/unified-secops/submission-guide); signing does not override an antivirus verdict.
